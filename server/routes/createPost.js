...
router.post("/", (req, res) => {
  ...
  const query = SqlString.format(
    "INSERT INTO forumPosts (userPosted, categoryId, title, contents, likes, dislikes) VALUES (?, ?, ?, ?, ?, ?)"
  )

  db.query(
    query,
    [userPosted, categoryId, title, contents, likes || 0, dislikes || 0],
    (err, result) => {
      if (err) {
